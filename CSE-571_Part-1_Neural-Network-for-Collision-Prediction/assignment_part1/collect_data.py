    ...
    for action_i in range(total_actions):
        ...
        for action_timestep in range(action_repeat):
            ...
            if collision:
                ...
                if action_timestep < action_repeat * 0.3: #in case prior action caused collision
                    network_params[-1][-1] = collision #share collision result with prior action
                break

        dataSet = list(sensor_readings) + [action] + [int(collision)]
        network_params.append(dataSet)

        if action_i == total_actions - 1:
            break

    with open('saved/training_data.csv', mode = 'w', newline='') as file:
